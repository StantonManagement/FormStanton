...
export default function LobbyPage() {
  ...
  const [showReissueConfirm, setShowReissueConfirm] = useState<boolean>(false);
  const [reissueTowOld, setReissueTowOld] = useState<boolean>(false);
  const [showPermitHistory, setShowPermitHistory] = useState<boolean>(false);
  ...
  const handleReissuePermit = async () => {
    if (!activeTenant || !activeTenant.submissionData) return;
    ...
    try {
      ...
      if (data.success) {
        ...
        setReissueTowOld(false);
        setShowPermitHistory(false);
      } else {
        setAlertDialog({ isOpen: true, title: 'Re-issue Failed', message: data.message, variant: 'error' });
      }
    } catch (e) {
      setAlertDialog({ isOpen: true, title: 'Re-issue Failed', message: 'Re-issue failed', variant: 'error' });
    } finally {
      setUpdatingField(null);
    }
  };
            ...
            {sub.has_vehicle ? (
              <>
                ...
                <div className="p-4 bg-[var(--bg-section)] border border-[var(--divider)]">
                  ...
                  {sub.permit_revoked && (
                    <div className="mb-3 p-3 bg-[var(--error)]/10 border border-[var(--error)]/40">
                      ...
                      <div className="mt-2 flex items-center gap-3">
                        ...
                        <button
                          onClick={() => { setShowReissueConfirm(true); setReissueTowOld(false); }}
                      ...
                      {showReissueConfirm && (
                        <div className="mt-2 p-3 bg-white border border-[var(--border)] space-y-2">
                          ...
                          <label className="flex items-center gap-2 text-xs text-[var(--ink)] cursor-pointer">
                            <input
                              type="checkbox"
                              checked={reissueTowOld}
                              onChange={(e) => setReissueTowOld(e.target.checked)}
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => { setShowReissueConfirm(false); setReissueTowOld(false); }}
                  ...
                  {/* Permit history (archived cycles) */}
                  {Array.isArray(sub.permit_history) && sub.permit_history.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-[var(--divider)]">
                      <button
                        type="button"
                      ...
                      >
                        {showPermitHistory ? '▾' : '▸'} Permit history ({sub.permit_history.length} previous cycle{sub.permit_history.length === 1 ? '' : 's'})
                      </button>
                      {showPermitHistory && (
                        <div className="mt-2 space-y-2">
                          {sub.permit_history.map((cycle, idx) => (
                            <div key={idx} className="p-2 bg-white border border-[var(--divider)] text-xs space-y-0.5">
                              ...
                              {cycle.tow_flagged && (
                                <div className="text-[var(--error)]">Tow order issued{cycle.towed_at && <> — towed {new Date(cycle.towed_at).toLocaleDateString()}</>}</div>
                              )}
