  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      if (key.startsWith('photo_') && value instanceof File) {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('form-photos')
          .upload(`unauthorized-pet/${fileName}`, file, {
    ...
    if (signature) {
      ...
      const { data: sigUploadData, error: sigUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`unauthorized-pet/${signatureFileName}`, signatureBuffer, {
    ...
    try {
      ...
      const pdfFileName = `${submission.id}_unauthorized_pet.pdf`;
      const { data: pdfUpload, error: pdfUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`unauthorized-pet/${pdfFileName}`, formPdf, {
    ...
    return NextResponse.json(
      { message: 'Unauthorized pet notice acknowledgment submitted successfully', id: submission.id, pdfGenerated: !!pdfPath },
      { status: 200 }
    );
  } catch (error: any) {
    console.error('Unauthorized pet submission error:', error);
    return NextResponse.json(
