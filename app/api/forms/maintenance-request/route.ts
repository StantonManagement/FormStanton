  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      if (key.startsWith('photo_') && value instanceof File) {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('form-photos')
          .upload(`maintenance-request/${fileName}`, file, {
    ...
    if (signature) {
      ...
      const { data: sigUploadData, error: sigUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`maintenance-request/${signatureFileName}`, signatureBuffer, {
    ...
    try {
      ...
      const pdfFileName = `${submission.id}_maintenance_request.pdf`;
      const { data: pdfUpload, error: pdfUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`maintenance-request/${pdfFileName}`, formPdf, {
    ...
    return NextResponse.json(
      { message: 'Maintenance request submitted successfully', id: submission.id, pdfGenerated: !!pdfPath },
      { status: 200 }
    );
  } catch (error: any) {
