  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      if (key.startsWith('photo_') && value instanceof File) {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('form-photos')
          .upload(`bulk-disposal/${fileName}`, file, {
    ...
    if (signature) {
      ...
      const { data: sigUploadData, error: sigUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`bulk-disposal/${signatureFileName}`, signatureBuffer, {
    ...
    try {
      ...
      const pdfFileName = `${submission.id}_bulk_disposal.pdf`;
      const { data: pdfUpload, error: pdfUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`bulk-disposal/${pdfFileName}`, formPdf, {
    ...
    return NextResponse.json(
      { message: 'Bulk disposal request submitted successfully', id: submission.id, pdfGenerated: !!pdfPath },
      { status: 200 }
    );
  } catch (error: any) {
    console.error('Bulk disposal submission error:', error);
    return NextResponse.json(
