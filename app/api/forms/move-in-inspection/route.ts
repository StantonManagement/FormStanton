...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const pdfFileName = `${submission.id}_move_in_inspection.pdf`;
      const { data: pdfUpload, error: pdfUploadError } = await supabase.storage
        .from('form-photos')
        .upload(`move-in-inspection/${pdfFileName}`, formPdf, {
    ...
    return NextResponse.json(
      { message: 'Move-in inspection submitted successfully', id: submission.id, pdfGenerated: !!pdfPath },
      { status: 200 }
    );
  } catch (error: any) {
