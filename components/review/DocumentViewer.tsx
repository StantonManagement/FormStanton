...
export default function DocumentViewer({ document: doc, context, onClose }: DocumentViewerProps) {
  ...
  const fileType = inferFileType(activeFileName);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-5" onClick={onClose}>
      <div className="bg-white w-full max-w-4xl max-h-screen flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-start flex-shrink-0">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-gray-900 truncate">{doc.label}</h3>
            {activeFileName && (
              <p className="text-sm text-gray-500 font-mono mt-1 truncate">{activeFileName}</p>
            )}
          </div>
          <button
            onClick={onClose}
            className="ml-4 px-3 py-1.5 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50"
          >
            Close ✕
          </button>
        </div>
        ...
        {/* Version tabs */}
        {hasMultiple && (
          <div className="flex gap-2 px-6 py-3 border-b border-gray-200 bg-gray-50 flex-shrink-0">
            {revisions.map((rev) => {
              const isActive = rev.revision === activeRevision;
              return (
                <button
            ...
            {hasMultiple && (
              <div className="text-xs text-gray-400 self-center">← → to navigate</div>
            )}
          </div>
        )}
        ...
        <div className="flex-1 overflow-auto bg-gray-100 flex items-center justify-center min-h-0">
          ...
          {!loading && error && (
            <div className="text-red-700 bg-red-50 p-4 max-w-md border border-red-200">
              {error}
            </div>
          )}
          {!loading && !error && !activeUrl && (
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-gray-200 flex justify-between items-center bg-gray-50 flex-shrink-0">
