...
**PRD:** `11-appointment-scheduling_prd_2026-04-24.md` (read it first)
...
6. **Staff day view** at `/admin/scheduling/today` (and `/admin/scheduling?date=YYYY-MM-DD` for other days):
   ...
   - Per appointment card:
     ...
     - Purpose
     - Context: list of unsigned documents for this application (from `application_documents WHERE anchor_type = 'pbv_full_application' AND requires_signature = true AND status NOT IN ('approved','waived')` — PRD-11a decoupled from `form_submission_documents`)
