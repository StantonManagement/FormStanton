...
**Depends on:** `hach-auth` (Stanton-side scoping), `income-eligibility-engine` (math display), `hach-reviewer-portal` (document state being tracked), PRD-11a (PBV/form-submission decoupling ✅ complete)
...
- Opens dialog: "What are they missing?"
  - Checklist of outstanding documents (auto-populated from `application_documents WHERE anchor_type = 'pbv_full_application' AND status IN ('missing', 'rejected')` — PRD-11a decoupled from `form_submission_documents`)
