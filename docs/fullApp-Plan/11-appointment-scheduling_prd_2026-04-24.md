...
**Depends on:** `hach-auth` (staff auth), Twilio (for SMS confirmations/reminders), PRD-11a (PBV/form-submission decoupling ✅ complete)
...
| `pbv_full_applications` / `application_documents` | Read | Compute "what to sign" context per appointment (PRD-11a decoupled from `form_submission_documents`) |
